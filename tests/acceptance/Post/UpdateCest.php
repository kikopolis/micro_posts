...
class UpdateCest
{
	 ...
	 */
	public function editingPostWorks(AcceptanceTester $I)
	{
		$body = (string) implode(' ', Factory::create()->words(20));
		...
		$I->click('Edit');
		$I->fillField('post[body]', $body . '123');
		$I->click('#post_submit');
		$I->expect('form is submitted correctly');
		
		$I->see($clean . '123', 'p[class=text-dark]');
		$I->see('Microscopic Post with the id of', 'h1');
		$I->seeLink('@testUser');
		$I->seeLink('Edit');
		$I->seeLink('Trash');
		$I->see('Post edited successfully.', 'p');
	}
}